...
rp_module_id="image"
rp_module_desc="Create/Manage RetroEvolved images"
rp_module_section=""
rp_module_flags="!arm"
...
function install_rp_image() {
    ...
    # hostname to retroevolved
    echo "retroevolved" >chroot/etc/hostname
    sed -i "s/raspberrypi/retroevolved/" chroot/etc/hosts
...
git clone https://github.com/RetroEvolved/RetroEvolved-Setup.git
...
for module in "\${modules[@]}"; do
    ...
    sudo QEMU_CPU=cortex-a15 __platform=$platform __nodialog=1 ./retroevolved_packages.sh \$module
done
sudo apt-get clean
_EOF_
...
function create_image() {
    ...
    mkfs.ext4 -O ^metadata_csum,^huge_file -L retroevolved /dev/mapper/loop0p2
...
function platform_image() {
    ...
    if [[ "$platform" == "rpi1" ]]; then
        image="retroevolved-${__version}-rpi1_zero"
    else
        image="retroevolved-${__version}-rpi2_rpi3"
    fi
