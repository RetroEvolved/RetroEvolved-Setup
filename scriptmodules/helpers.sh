...
## @file helpers.sh
## @brief RetroEvolved helpers library
## @copyright GPLv3
...
function getDepends() {
    ...
    local own_sdl2=1
    iniConfig " = " '"' "$configdir/all/retroevolved.cfg"
    iniGet "own_sdl2"
...
# @fn setupDirectories()
# @brief Makes sure some required retroevolved directories and files are created.
function setupDirectories() {
    ...
    if [[ ! -f "$config" ]]; then
        echo "# this file can be used to enable/disable retroevolved autoconfiguration features" >"$config"
    fi
...
## @brief Creates a default retroarch.cfg for specified system in `/opt/retroevolved/configs/$system/retroarch.cfg`.
...
# added by RetroEvolved-Setup - $res mode for emulators
mode "$res"
...
function getPlatformConfig() {
    ...
    # workaround for RetroEvolved platform
    [[ "$key" == "retroevolved_fullname" ]] && ini_value="RetroEvolved"
    echo "$ini_value"
}
