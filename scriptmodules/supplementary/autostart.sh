...
function _autostart_script_autostart() {
    ...
    cat >/etc/profile.d/10-retroevolved.sh <<_EOF_
...
function enable_autostart() {
    ...
    if isPlatform "x11"; then
        ...
        ln -sf "/usr/local/share/applications/retroevolved.desktop" "$home/.config/autostart/"
    else
...
function disable_autostart() {
    ...
    if isPlatform "x11"; then
        rm "$home/.config/autostart/retroevolved.desktop"
    else
        if [[ "$__os_id" == "Raspbian" ]]; then
            if [[ "$__chroot" -eq 1 ]]; then
        ...
        rm -f /etc/profile.d/10-emulationstation.sh
        rm -f /etc/profile.d/10-retroevolved.sh
    fi
}
...
function gui_autostart() {
    ...
    while true; do
        if isPlatform "x11"; then
            local x11_autostart
            if [[ -f "$home/.config/autostart/retroevolved.desktop" ]]; then
