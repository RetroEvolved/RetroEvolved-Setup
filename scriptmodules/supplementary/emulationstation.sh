...
rp_module_desc="EmulationStation - Frontend used by RetroEvolved for launching emulators"
rp_module_licence="MIT https://raw.githubusercontent.com/RetroEvolved/EmulationStation/master/LICENSE.md"
...
function sources_emulationstation() {
    local repo="$1"
    local branch="$2"
    [[ -z "$repo" ]] && repo="https://github.com/RetroEvolved/EmulationStation"
...
if [[ $? -eq 139 ]]; then
    dialog --cr-wrap --no-collapse --msgbox "Emulation Station crashed!\n\nIf this is your first boot of RetroEvolved - make sure you are using the correct image for your system.\n\\nCheck your rom file/folder permissions and if running on a Raspberry Pi, make sure your gpu_split is set high enough and/or switch back to using the stock RetroEvolved theme.\n\n" 20 60 >/dev/tty
fi
tput cnorm
_EOF_
    ...
    if isPlatform "x11"; then
        ...
        cp "$md_data/retroevolved.svg" "/usr/local/share/icons/"
        cat > /usr/local/share/applications/retroevolved.desktop << _EOF_
[Desktop Entry]
Type=Application
...
Name[de_DE]=RetroEvolved
Name=rpie
Comment[de_DE]=RetroEvolved
Comment=retroevolved
Icon=/usr/local/share/icons/retroevolved.svg
Categories=Game
_EOF_
    fi
}
...
function remove_emulationstation() {
    ...
    if isPlatform "x11"; then
        rm -rfv "/usr/local/share/icons/retroevolved.svg" "/usr/local/share/applications/retroevolved.desktop"
    fi
}
