...
function install_bin_splashscreen() {
    ...
    gitPullOrClone "$md_inst" https://github.com/RetroEvolved/retroevolved-splashscreens.git
...
function default_splashscreen() {
    echo "$md_inst/retroevolved-default.png" >/etc/splashscreen.list
}
...
function randomize_splashscreen() {
    options=(
        1 "Randomize RetroEvolved splashscreens"
...
function download_extra_splashscreen() {
    gitPullOrClone "$datadir/splashscreens/retroevolved-extra" https://github.com/HerbFargus/retroevolved-splashscreens-extra
    chown -R $user:$user "$datadir/splashscreens/retroevolved-extra"
}
...
function gui_splashscreen() {
    ...
    while true; do
        ...
        options+=(
            ...
            8 "Update RetroEvolved splashscreens"
            9 "Download RetroEvolved-Extra splashscreens"
        )
        local choice=$("${cmd[@]}" "${options[@]}" 2>&1 >/dev/tty)
        if [[ -n "$choice" ]]; then
            case "$choice" in
                ...
                4)
                    ...
                    printMsgs "dialog" "Splashscreen set to RetroEvolved default."
                    ;;
                5)
                ...
                9)
                    ...
                    printMsgs "dialog" "The RetroEvolved-Extra splashscreens have been downloaded to $datadir/splashscreens/retroevolved-extra"
                    ;;
            esac
        else
            break
