...
rp_module_desc="RetroEvolved-Setup Wiki Viewer"
...
function gui_wikiview() {
    local wikidir="$rootdir/RetroEvolved-Setup.wiki"
    while true; do
        local cmd=(dialog --backtitle "$__backtitle" --menu "RetroEvolved-Setup Wiki Viewer" 22 76 16)
        local options=()
        if [[ -d "$wikidir" ]]; then
            options=(
                1 "Update RetroEvolved-Setup Wiki"
                2 "View Wiki Pages"
                3 "Remove RetroEvolved-Setup Wiki"
            )
        else
            options+=("1" "Download RetroEvolved-Setup Wiki")
        fi
        local choice=$("${cmd[@]}" "${options[@]}" 2>&1 >/dev/tty)
        local file="-"
        if [[ -n "$choice" ]]; then
            case "$choice" in
                1)
                    gitPullOrClone "$wikidir" "https://github.com/RetroEvolved/RetroEvolved-Setup.wiki.git"
                    ;;
                2)
                    while [[ -n  "$file" ]]; do
                        file=""
