...
function depends_sdl2() {
    ...
    # already covered by the build-essential package retroevolved relies on.
...
function sources_sdl2() {
    ...
    isPlatform "rpi" && branch="retroevolved-$ver"
    isPlatform "mali" && branch="mali-$ver"

    gitPullOrClone "$md_build/$pkg_ver" https://github.com/RetroEvolved/SDL-mirror.git "$branch"
    cd "$pkg_ver"
