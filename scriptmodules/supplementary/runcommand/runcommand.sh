...
## will be extracted from `/opt/retroevolved/configs/SYSTEM/emulators.cfg` with
## `%ROM%` `%BASENAME%` being replaced with the ROM parameter. This is the
## default mode used when launching in RetroEvolved so the user can switch emulator
...
function get_sys_command() {
    ...
    # see https://github.com/RetroEvolved/RetroEvolved-Setup/issues/1805
...
function show_launch() {
    ...
    if [[ "$IS_SYS" -eq 1 && "$USE_ART" -eq 1 ]]; then
        ...
        images+=(
            "$HOME/RetroEvolved/roms/$SYSTEM/images/${ROM_BN}-image"
    ...
    if [[ "$IS_SYS" -eq 1 ]]; then
        images+=(
            "$HOME/RetroEvolved/roms/$SYSTEM/images/${ROM_BN}-launching"
            "$CONF_ROOT/launching"
        )
    fi
